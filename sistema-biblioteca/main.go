...
func crearLibro(w http.ResponseWriter, r *http.Request) {
	...
	query := `INSERT INTO Libros (Titulo, Autor, Categoria, Año, Prestado, Precio) VALUES (?, ?, ?, ?, ?, ?)`
	_, err = db.Exec(query, nuevoLibro.Titulo, nuevoLibro.Autor, nuevoLibro.Categoria, nuevoLibro.Año, nuevoLibro.Prestado, nuevoLibro.Precio)
	if err != nil {
		http.Error(w, "Error al crear el libro", http.StatusInternalServerError)
		log.Println("Error al crear el libro:", err)
		return
	}
...
func actualizarLibro(w http.ResponseWriter, r *http.Request) {
	...
	query := `UPDATE Libros SET Titulo = ?, Autor = ?, Categoria = ?, Año = ?, Prestado = ?, Precio = ? WHERE LibroID = ?`
	_, err = db.Exec(query, libro.Titulo, libro.Autor, libro.Categoria, libro.Año, libro.Prestado, libro.Precio, libroID)
	if err != nil {
	...
	w.Write([]byte("Libro actualizado con éxito"))
}
...
func eliminarLibro(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte("Libro eliminado con éxito"))
}
